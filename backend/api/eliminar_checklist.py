...
def delete_checklist():
    try:
        ...
        response = requests.get(
            SUPABASE_URL + "maquinas?codigo_interno=eq." + codigo_interno,
            headers=HEADERS,
        )
        ...
        if response.status_code != 200 or not response.json():
            return (
                jsonify(
                    {"error": "Checklist no encontrado para la máquina especificada"}
                ),
                404,
            )
        ...
        response = requests.get(
            SUPABASE_URL + "componentes?id_checklist=eq." + str(checklist_id),
            headers=HEADERS,
        )
        ...
        for componente in componentes:
            ...
            response = requests.get(
                SUPABASE_URL + "tareas?id_componente=eq." + str(componente_id),
                headers=HEADERS,
            )
            if response.status_code != 200:
            ...
            for tarea in tareas:
                ...
                response = requests.delete(
                    SUPABASE_URL + "tareas?id_tarea=eq." + str(tarea_id),
                    headers=HEADERS,
                )
            ...
            response = requests.delete(
                SUPABASE_URL + "componentes?id_componente=eq." + str(componente_id),
                headers=HEADERS,
            )
            if response.status_code != 204:
        ...
        response = requests.delete(
            SUPABASE_URL + "checklists_maquinas?id_checklist=eq." + str(checklist_id),
            headers=HEADERS,
        )
        if response.status_code != 204:
            return (
                jsonify(
                    {"error": "Error eliminando asignación del checklist a la máquina"}
                ),
                500,
            )
        ...
        response = requests.delete(
            SUPABASE_URL + "checklists?id_checklist=eq." + str(checklist_id),
            headers=HEADERS,
        )
        if response.status_code != 204:
        ...
        return (
            jsonify(
                {
                    "message": "Checklist y sus componentes y tareas eliminados exitosamente"
                }
            ),
            200,
        )
