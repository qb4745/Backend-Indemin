...
CORS(estado_checklist_bp, origins=['http://localhost:8100', 'http://127.0.0.1:5500', 'https://alvarofenero.github.io'], supports_credentials=True)
...
def realizar_checklist():
    try:
        ...
        if not id_checklist or not id_usuario or not fecha_realizacion or not estados_tareas:
        ...
        response = requests.post(SUPABASE_URL + 'checklists_realizados', headers=HEADERS, data=json.dumps(checklist_realizado_data))
        if response.status_code != 201:
            print('Error al crear la realización del checklist:', response.text)
            return jsonify({'message': 'Error al crear la realización del checklist'}), 500
        ...
        if not id_checklist_realizado:
            response = requests.get(SUPABASE_URL + f'checklists_realizados?id_checklist=eq.{id_checklist}&id_usuario=eq.{id_usuario}&fecha_realizacion=eq.{fecha_realizacion}', headers=HEADERS)
            if response.status_code != 200 or not response.json():
                print('No se pudo obtener el ID del checklist realizado.')
                return jsonify({'message': 'No se pudo obtener el ID del checklist realizado'}), 500
        ...
        for estado_tarea in estados_tareas:
            ...
            response = requests.post(SUPABASE_URL + 'tarea_estado', headers=HEADERS, data=json.dumps(estado_tarea_data))
            ...
            if response.status_code != 201:
                ...
                return jsonify({'message': 'Error al guardar el estado de las tareas'}), 500
...
def obtener_checklists_realizados():
    try:
        ...
        offset = (page - 1) * size
        response = requests.get(f"{SUPABASE_URL}checklists_realizados?offset={offset}&limit={size}", headers=HEADERS)
        if response.status_code != 200:
            return jsonify({'message': 'Error al obtener los checklists realizados'}), 500

        checklists_realizados = response.json()
        total_items_response = requests.get(f"{SUPABASE_URL}checklists_realizados?select=count", headers=HEADERS)
        total_items = total_items_response.json()[0]['count']
        total_pages = (total_items + size - 1) // size

        for checklist in checklists_realizados:
            user_response = requests.get(SUPABASE_URL + f'usuario?id_usuario=eq.{checklist["id_usuario"]}', headers=HEADERS)
            if user_response.status_code == 200:
                checklist['usuario'] = user_response.json()[0]['email']

            checklist_id = checklist['id_checklist']
            checklist_maquina_response = requests.get(SUPABASE_URL + f'checklists_maquinas?id_checklist=eq.{checklist_id}', headers=HEADERS)
            if checklist_maquina_response.status_code == 200 and checklist_maquina_response.json():
                id_maquina = checklist_maquina_response.json()[0]['id_maquina']
                maquina_response = requests.get(SUPABASE_URL + f'maquinas?id_maquina=eq.{id_maquina}', headers=HEADERS)
                if maquina_response.status_code == 200:
                    ...
                    faena_response = requests.get(SUPABASE_URL + f'faenas?id_faena=eq.{maquina_data["id_faena"]}', headers=HEADERS)
                    if faena_response.status_code == 200:
                        checklist['faena'] = faena_response.json()[0]['nombre']

        return jsonify({
            'items': checklists_realizados,
