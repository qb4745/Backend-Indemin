...
def get_checklists():
    try:
        ...
        response = requests.get(
            f"{SUPABASE_URL}maquinas?codigo_interno=eq.{codigo_interno}",
            headers=HEADERS,
        )
        ...
        response = requests.get(
            f"{SUPABASE_URL}checklists_maquinas?id_maquina=eq.{id_maquina}",
            headers=HEADERS,
        )
        ...
        if response.status_code != 200:
            return (
                jsonify({"error": "No se pudieron obtener los checklists asociados"}),
                500,
            )
        ...
        for checklist_id in checklist_ids:
            response = requests.get(
                f"{SUPABASE_URL}checklists?id_checklist=eq.{checklist_id}",
                headers=HEADERS,
            )
            if response.status_code == 200 and response.json():
                ...
                response_componentes = requests.get(
                    f"{SUPABASE_URL}componentes?id_checklist=eq.{checklist_id}",
                    headers=HEADERS,
                )
                if (
                    response_componentes.status_code == 200
                    and response_componentes.json()
                ):
                    componentes = response_componentes.json()
                    for componente in componentes:
                        componente_id = componente["id_componente"]
                        # Obtener tareas del componente
                        response_tareas = requests.get(
                            f"{SUPABASE_URL}tareas?id_componente=eq.{componente_id}",
                            headers=HEADERS,
                        )
                        if (
                            response_tareas.status_code == 200
                            and response_tareas.json()
                        ):
        ...
        return (
            jsonify({"error": "Error al parsear la respuesta", "details": str(e)}),
            500,
        )
