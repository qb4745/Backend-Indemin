...
CORS(crear_usuario_bp, origins=['http://localhost:8100', 'http://127.0.0.1:5500', 'https://alvarofenero.github.io'], supports_credentials=True)
...
def crear_usuario():
    try:
        ...
        response = requests.post(SUPABASE_URL + 'usuario', headers=HEADERS, data=json.dumps(usuario_data))
...
def editar_permisos(current_user, id_usuario):
    try:
        ...
        response = requests.patch(SUPABASE_URL + 'usuario?id_usuario=eq.' + str(id_usuario), headers=HEADERS, data=json.dumps(usuario_data))
        if response.status_code == 204:
            return jsonify({'message': 'Permisos de usuario actualizados exitosamente'}), 200
        else:
...
def eliminar_usuario(current_user, id_usuario):
    try:
        response = requests.delete(SUPABASE_URL + 'usuario?id_usuario=eq.' + str(id_usuario), headers=HEADERS)
        if response.status_code == 204:
