...
def crear_maquinas():
    ...
    try:
        ...
        if id_faena is None or id_faena == 0:
            ...
            if id_faena is None:
                return (
                    jsonify({"error": "No se pudo crear la faena automáticamente"}),
                    500,
                )
        ...
        maquinas_logger.debug(
            f"Respuesta de Supabase al crear máquina: {response.status_code} - {response.text}"
        )
        ...
        if response.status_code == 201:
            ...
            return (
                jsonify({"error": "No se pudo crear la máquina"}),
